...
const renderFormattedLine = (line: string, lineIndex: number): JSX.Element => {
  ...
  let textColorClass = 'text-gray-200'; // Default
...
const Slide: React.FC<SlideProps> = ({ slide }) => {
  ...
  return (
    <div className="w-full h-full flex flex-col animate-fadeIn overflow-hidden p-4 sm:p-5">
      <h2 className="text-xl sm:text-2xl font-bold text-purple-400 mb-4 text-center truncate flex-shrink-0">
        {slide.title}
      </h2>
      ...
      <div className="flex-1 w-full flex flex-col items-stretch overflow-hidden gap-4 min-h-0">
        ...
        >
          {slide.type === SlideType.USER_CODE ? (
            <div className="w-full h-full bg-gray-900/80 p-3 sm:p-4 rounded-lg shadow-inner overflow-auto custom-scrollbar border border-gray-700">
              <pre className="text-left text-sm text-gray-300 whitespace-pre-wrap font-fira-code leading-relaxed">
             ...
             <div className="text-base sm:text-lg leading-relaxed space-y-3 prose prose-invert max-w-none text-center p-2">
              {contentParts.length > 0 ? (
                ...
                <p className="italic text-gray-400">No display content available for this slide.</p> 
              )}
            </div>
          )}
        </div>
        ...
        {hasDiagram && (
          ...
          <div className="w-full flex-1 flex items-center justify-center border-t border-gray-700/50 pt-4 min-h-0">
            <div 
