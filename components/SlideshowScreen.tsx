...
const SlideshowScreen: React.FC<SlideshowScreenProps> = ({ slides, onReset }) => {
  ...
  return (
    <div className="min-h-screen w-full flex flex-col items-center justify-center p-2 sm:p-4 bg-gray-900">
      <audio ref={audioRef} onEnded={handleAudioEnded} />

      <div className="w-full bg-gray-700/50 h-1.5 fixed top-0 left-0 z-50 shadow-lg">
        <div
          className="bg-gradient-to-r from-purple-600 to-pink-500 h-1.5 transition-all duration-300 ease-linear"
          style={{ width: `${progressPercentage}%` }}
        ></div>
      </div>

      {/* Main Content Area: Vertical Card */}
      <div className="w-full max-w-md aspect-[9/16] bg-gray-800/50 rounded-2xl shadow-2xl overflow-hidden border border-gray-700/50">
        ...
        <div className="flex justify-center items-center space-x-4 mt-4">
          ...
          {currentSlideIndex === slides.length - 1 ? (
            <Button 
              onClick={onReset} 
              variant="secondary"
              className="w-1/4 bg-green-600 hover:bg-green-700 focus:ring-green-500"
         ...
         <div className="text-center mt-6">
            <Button onClick={onReset} variant="secondary" className="!bg-transparent !text-gray-400 hover:!bg-gray-700 !shadow-none !py-2 !px-4">
              Start Over
            </Button>
         </div>
      </div>
