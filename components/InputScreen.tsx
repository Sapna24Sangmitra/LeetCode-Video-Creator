...
const InputScreen: React.FC<InputScreenProps> = ({ onSubmit, isLoading }) => {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-4 sm:p-6 lg:p-8">
      <div className="w-full max-w-3xl bg-indigo-950/80 backdrop-blur-md p-8 sm:p-10 rounded-xl shadow-2xl border border-indigo-700/50">
        <h1 className="text-4xl sm:text-5xl font-extrabold text-center mb-6 bg-gradient-to-r from-yellow-400 via-yellow-300 to-yellow-200 text-transparent bg-clip-text">
          LeetCode Gemini Explainer
        </h1>
        <p className="text-center text-yellow-200 mb-10 text-base sm:text-lg leading-relaxed">
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          <div>
            <label htmlFor="dayNumber" className="block text-base font-medium text-yellow-300 mb-2">
            ...
            <input
              ...
              required
              disabled={isLoading}
              className="w-full p-3.5 bg-indigo-800/50 border border-indigo-600 rounded-lg shadow-sm text-yellow-200 placeholder-yellow-500/70 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 focus:bg-indigo-800/70 transition-colors duration-150 ease-in-out"
            />
          </div>
          <TextArea
            id="problemStatement"
       ...
       <footer className="mt-12 text-center text-yellow-600">
        <p>&copy; {new Date().getFullYear()} LeetCode Gemini Explainer. Powered by Google Gemini.</p>
        <p className="mt-1">Ensure your <code className="bg-indigo-800/50 px-1.5 py-0.5 rounded-md font-fira-code text-xs">API_KEY</code> environment variable is set.</p>
      </footer>
    </div>
  );
};
